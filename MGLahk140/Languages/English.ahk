...
MG_LngOthers003 = Function and Subroutine Definitions
...
MG_LngOthers007 = Place the process you want to execute when the initialization of MouseGestureL.ahk is completed below.
MG_LngOthers008 = Place the process you want to execute when the initialization of MG_Edit.ahk is completed below.
...
ActionComment062 = Move window to ###REPLACE###
...
ME_LngCapt020 = Establish Move Window
...
ME_LngText591 = Specify the destination of the window.
ME_LngText592 = - If you select "Original Position" as the destination, the window moved`n  immediately before by this command will be returned to the position`n  before the move.
ME_LngText593 = - In a multi-display environment, you can move a window to the sub monitors`n  by specifying the monitor number.
ME_LngText594 = - You can fine-tune the destination position by specifying the offset value.
ME_LngText595 = Destination:
ME_LngText596 = Monitor #:
ME_LngText597 = X:
ME_LngText598 = Y:
...
ME_LngDropDown211 = Screen Center`nScreen Left`nScreen Top`nScreen Right`nScreen Bottom`nScreen Left-Top`nScreen Right-Top`nScreen Left-Bottom`nScreen Right-Bottom`nOriginal Position
...
ME_LngGroupBox103 = Offset of destination coordinates
...
ME_LngCheckBox113 = Adjust position for the Windows Aero
...
[RB_]
G=RB_
Default=;Activate Window<MG_CR>if (MG_WClass != "Chrome_WidgetWin_2") {<MG_CR><MG_TAB>MG_WinActivate()<MG_CR>}
